...
class Var:
    ...
    if not BOT_TOKEN or not API_HASH or not API_ID or not MONGO_URI:
        LOGS.critical(
            'Important Variables Missing. Fill Up and Retry..!! Exiting Now...'
        )
        exit(1)
    ...
    FSUB_CHATS = list(
        map(int, getenv('FSUB_CHATS', "").split())
    )
    ...
    ADMINS = list(
        map(int, getenv("ADMINS", "1242011540").split())
    )

    SEND_SCHEDULE = getenv(
        "SEND_SCHEDULE",
        "False"
    ).lower() == "true"
    ...
    FFCODE_1080 = getenv("FFCODE_1080") or (
        """ffmpeg -i '{}' -progress '{}' -preset veryfast """
        """-c:v libx264 -s 1920x1080 -pix_fmt yuv420p """
        """-crf 30 -c:a libopus -b:a 32k -c:s copy """
        """-map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 '{}' -y"""
    )

    FFCODE_720 = getenv("FFCODE_720") or (
        """ffmpeg -i '{}' -progress '{}' -preset superfast """
    ...
    FFCODE_480 = getenv("FFCODE_480") or (
        """ffmpeg -i '{}' -progress '{}' -preset superfast """
        """-c:v libx264 -s 854x480 -pix_fmt yuv420p """
        """-crf 30 -c:a libopus -b:a 32k -c:s copy """
        """-map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 '{}' -y"""
    )

    FFCODE_360 = getenv("FFCODE_360") or (
        """ffmpeg -i '{}' -progress '{}' -preset superfast """
        """-c:v libx264 -s 640x360 -pix_fmt yuv420p """
        """-crf 30 -c:a libopus -b:a 32k -c:s copy """
        """-map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 '{}' -y"""
    )
    ...
    THUMB = getenv(
        "THUMB",
        "https://te.legra.ph/file/621c8d40f9788a1db7753.jpg"
    )
    ...
    START_PHOTO = getenv(
        "START_PHOTO",
        "https://te.legra.ph/file/120de4dbad87fb20ab862.jpg"
    )
    ...
    START_BUTTONS = getenv(
        "START_BUTTONS",
        "UPDATES|https://telegram.me/Matiz_Tech "
        "SUPPORT|https://t.me/+p78fp4UzfNwzYzQ5"
    )
